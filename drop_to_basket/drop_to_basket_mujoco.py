...
def build_targets(
    ...
    arm_interpolator: ArmInterpolator,
    ...
    if not arm_interpolator.is_done(move_step):
        targets.update(arm_interpolator.get_target(move_step))
        return targets, False

    targets.update(arm_interpolator.get_target(arm_interpolator.total_steps))

    hand_step = move_step - arm_interpolator.total_steps
    if not hand_interpolator.is_done(hand_step):
        targets.update(hand_interpolator.get_target(hand_step))
        return targets, False
...
def run_demo(args: argparse.Namespace) -> None:
    ...
    move_duration_s = estimate_move_duration_s(arm_start, drop_pose)
    arm_interpolator = ArmInterpolator(
        arm_start, drop_pose, move_duration_s, round(1.0 / SIMULATE_DT)
    )
    hand_interpolator = ArmInterpolator(
        hand_closed, hand_open, GRIPPER_OPEN_DURATION_S, round(1.0 / SIMULATE_DT)
    )
    ...
    for name, value in drop_pose.items():
        print(f"  {name}: {value:.4f}")
    print(
        f"move duration: {move_duration_s:.2f}s "
...
def run_loop(
    ...
    while viewer is None or viewer.is_running():
        ...
        targets, done = build_targets(
            step, hold_targets, arm_interpolator, hand_interpolator, wait_steps
        )
        if dynamic:
