...
def load_target_pose(path: Path) -> dict[str, float]:
    ...
    if "right_arm_drop_pose" in payload:
        target = payload["right_arm_drop_pose"]
    else:
        target = payload

    missing = set(RIGHT_ARM_JOINTS) - set(target)
    extra = set(target) - set(RIGHT_ARM_JOINTS)
    if missing or extra:
        raise ValueError(
            "target pose keys must match right arm joints "
            f"(missing={sorted(missing)}, extra={sorted(extra)})"
        )
    return {joint_name: float(target[joint_name]) for joint_name in RIGHT_ARM_JOINTS}
