...
def _send_zero_moment(
    pub: ChannelPublisher, crc: CRC, mode_machine: int, duration_s: float = 2.0
) -> None:
    """Send zero torque to all arm joints — joints go passive before releasing arm_sdk."""
    print("[ZERO MOMENT] Sending zero torque to all arm joints...")
    cmd = unitree_hg_msg_dds__LowCmd_()
    cmd.mode_pr = 0
    cmd.mode_machine = mode_machine
    cmd.motor_cmd[kNotUsedJoint].q = 1.0
    ...
    while time.monotonic() < until:
        ...
        if elapsed < period:
            time.sleep(period - elapsed)
    print("[ZERO MOMENT] All arm joints passive.")
...
def main() -> None:
    ...
    try:
        ...
        print("Press Ctrl+C to enter zero moment and exit safely.")
        print("="*60)

        # Hold final rest position until user presses Ctrl+C
        final_rest = {k: 0.0 for k in ARM_IDX}
        final_rest["left_shoulder_roll"] = LEFT_ARM_REST_POSE["left_shoulder_roll"]
        try:
            while True:
                _hold(pub, crc, final_rest, mode_machine, 0.5, waist_q)
        except KeyboardInterrupt:
            print("\nCtrl+C — entering zero moment...")
            _send_zero_moment(pub, crc, mode_machine, duration_s=2.0)
