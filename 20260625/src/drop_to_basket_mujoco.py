...
# Fixed L-shape pose for left arm: elbow 90°, wrist neutral, basket faces up naturally
LEFT_ARM_L_SHAPE_POSE = {
    "left_shoulder_pitch_joint": -0.1170,
    "left_shoulder_roll_joint":  -0.0167,
    "left_shoulder_yaw_joint":   -0.3997,
    "left_elbow_joint":           1.1330,
    "left_wrist_roll_joint":      0.0834,
    "left_wrist_pitch_joint":    -1.0673,
    "left_wrist_yaw_joint":      -0.2355,
}
...
def solve_right_arm_drop_pose(
    ...
    try:
        ...
        target_pos = left_hand_pos + np.array([0.0, RIGHT_DROP_Y_OFFSET_M, DROP_HEIGHT_ABOVE_LEFT_HAND_M])
        ...
        solved.update(RIGHT_DROP_WRIST_DOWN)
...
def build_targets(
    ...
    phase4_step = phase3_step - right_to_drop.total_steps
    ...
    # Phase 8: left arm returns to normal
    phase8_step = phase7_step - right_to_park.total_steps
    if not left_to_normal.is_done(phase8_step):
        targets.update(left_to_normal.get_target(phase8_step))
        return targets, False, "phase8_left_return"

    targets.update(left_to_normal.get_target(left_to_normal.total_steps))

    # Phase 9: right arm returns to normal from park
    phase9_step = phase8_step - left_to_normal.total_steps
...
def run_demo(args: argparse.Namespace) -> None:
    ...
    left_return_duration_s  = estimate_move_duration_s(LEFT_ARM_L_SHAPE_POSE,  left_arm_rest)
    right_return_duration_s = estimate_move_duration_s(RIGHT_ARM_PARK_POSE,    right_arm_zeros)
    ...
    left_to_normal   = ArmInterpolator(LEFT_ARM_L_SHAPE_POSE,  left_arm_rest,           left_return_duration_s,  steps_per_s)
    right_to_normal  = ArmInterpolator(RIGHT_ARM_PARK_POSE,    right_arm_zeros,          right_return_duration_s, steps_per_s)
    ...
    if args.no_viewer:
        run_loop(
            ...
            hand_open_for_drop, gripper_hold_open_drop_steps,
            hand_close_after_drop,
            drop_to_approach, right_to_park, left_to_normal, right_to_normal,
...
def run_loop(
    ...
    while viewer is None or viewer.is_running():
        if _stop_requested[0]:
            print("\nCtrl+C — safe stopping...")
            try:
                _mujoco_safe_stop(model, data, refs, base_qpos, lock_base, dynamic, viewer)
        ...
        if phase != current_phase:
            ...
            elif phase == "phase8_left_return":
                print("→ Phase 8: left arm returning to normal")
